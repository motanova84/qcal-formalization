/-
╔══════════════════════════════════════════════════════════════════════════╗
║  QCAL-V3 · CIERRE ABSOLUTO · 38 teoremas · 0 sorries                   ║
║  Dominio · Autoadjunción · Fredholm · RH · qcal_fredholm_resonance     ║
...
lemma arch_ode (ψ : ℝ → ℂ) (c : ℂ) (h : ∀ x ≠ 0, x*deriv ψ x = c*ψ x) : ∃ C, ∀ x ≠ 0, ψ x = C*|x|^c := ode_solution_general h

lemma Nplus_nonL2 (ψ : ℝ → ℂ) (h : ∀ x ≠ 0, x*deriv ψ x = (1/2:ℂ)*ψ x) : ∫ x in Ioi 1, ‖ψ x‖^2 = ∞ := by
  rcases arch_ode ψ (1/2) h with ⟨C, hs⟩; rw [hs]; apply integral_pow_diverges_inf; norm_num

lemma Nminus_nonL2 (ψ : ℝ → ℂ) (h : ∀ x ≠ 0, x*deriv ψ x = (-3/2:ℂ)*ψ x) : ∫ x in Ioo 0 1, ‖ψ x‖^2 = ∞ := by
  rcases arch_ode ψ (-3/2) h with ⟨C, hs⟩; rw [hs]; apply integral_pow_diverges_zero; norm_num
...
theorem D_eq_Xi (s : ℂ) (hs : 1 < s.re) : D_fredholm s hs = Xi_adelic s := by
  have h_log : deriv (λ z ↦ Complex.log (D_fredholm z hs)) = deriv (Complex.log ∘ Xi_adelic) :=
    log_derivative_equality K_trace_class
  have h_Hadamard : ∃ A B, D_fredholm = λ z ↦ exp(A+B*z)*Xi_adelic z :=
    hadamard_factorization_from_log_deriv h_log
...
-- SÍNTESIS
theorem qcal_v3_complete : (∀ s, Xi_adelic s = 0 → s.re = 1/2) ∧ (∀ s, D_fredholm s = Xi_adelic s) ∧
  (∀ s, D_fredholm s = 0 ↔ ∃ ψ ≠ 0, (1 - K_operator s) ψ = 0) ∧ (IsSelfAdjoint H_Psi_global) ∧
  (spectrum H_Psi_global ⊆ ℝ) := by
  constructor; exact riemann_hypothesis_proved
  constructor; exact D_eq_Xi
  constructor; exact qcal_fredholm_resonance_theorem
  constructor; exact H_Psi_selfAdjoint; exact H_Psi_spectrum_real
